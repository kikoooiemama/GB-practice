 ...
 * Домашнее задание.
 * 
 * Сделал отдельные таблицы для лайков по отдельности для каждой категории (таблицы) users, media, posts. 
 * Понимаю не лучшее решение, поскольку база данных будет хранить каждый лайк отдельной записью и таблицы будут разрастаться.
 * 
 * Думал может быть как-то сделать счетчик, но пока не понимаю как.
 * Также была идея объеденить все таблицы: posts, media, users. Но я не понимаю как их связать с помощью REFERENCE.
 * Надеюсь вы мне посоветуете как можно было оптимально решить эту задачу.
 * 
 * Использовал уже созданные таблицы users, media. 
 * Создал новые таблицы likes_for_media, likes_for_users, likes_for_posts и posts.
 */
-- -------------------------------------------------------------------------------------------------------------------------

/**
 * Таблица лайков ко всем медиафайлам. 
 */
...
/**
 * Таблица лайков ко всем пользователям. 
 */
...
/**
 * Таблица всех постов всех пользователей.
 * 
 * Пост имеет свой id. У него есть to_user_id, на страничке которого оставлен пост. Есть автор поста from_user_id.
 * Также у поста есть текст сообщения и есть медиафайл. Если медиафайла нет, то значит он NULL.
 */
CREATE TABLE posts (
	id SERIAL PRIMARY KEY,
	from_user_id BIGINT UNSIGNED NOT NULL,
...
INSERT INTO posts VALUES (DEFAULT, 1, 1, 'Надо попросить Васю купить хлеба!', DEFAULT, DEFAULT, DEFAULT);
INSERT INTO posts VALUES (DEFAULT, 1, 2, 'Вася, купи хлеба!', 1, DEFAULT, DEFAULT);

/**
 * Таблица лайков ко всем постам всех пользователей. 
 */
